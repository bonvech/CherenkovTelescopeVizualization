...
from PyQt6.QtWidgets import QApplication, QDialog, QFileDialog, QMainWindow, QVBoxLayout, QLabel, QMenu, QPushButton, QWidget, QComboBox, QSpinBox, QDoubleSpinBox, QCheckBox, QMessageBox, QSlider, QPlainTextEdit, QTextBrowser, QHBoxLayout, QGroupBox, QGridLayout, QTableWidget, QTableWidgetItem, QHeaderView, QTableView, QAbstractScrollArea
...
class Appp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.timeofeventTabl = QTableWidget(self)
        self.timeofeventText.hide()
        ...
        hay = QHBoxLayout()
        self.qqq=QWidget()
        #hay.addWidget(self.qqq)
        hay.addWidget(self.timeofeventTabl)
        self.qq = QWidget()
        ...
        self.qqq = QWidget()
        ALL.addLayout(Allwidget)
        ALL.addWidget(self.qqq)
        self.centralwidget.setLayout(ALL)
    ...
    def boxEventwasChanged(self):
        ...
        #
        #self.timeofeventTabl.setModel(QtGui.QStandardItemModel(4, 4))
        #self.timeofeventTabl.
        #self.timeofeventTabl.horizontalHeader()#hide()
        
        self.timeofeventTabl.setColumnCount(3)
        self.timeofeventTabl.setHorizontalHeaderLabels(["st", "st_event", "st_time"])
        self.timeofeventTabl.setRowCount(len(s))
        self.ss = []
        for i in range(len(s)):
            for j in range(0, 3):
                q = QTableWidgetItem(f'{s[i].split()[j]}')
                q.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.timeofeventTabl.setItem(i, j, q)
                #ss.append()
            #self.timeofeventTabl.setItem(i, 1, QTableWidgetItem(f'{s[i].split()[1]}'))
            #self.timeofeventTabl.setItem(i, 0, QTableWidgetItem(f'{s[i].split()[0]}'))

        
        #self.timeofeventTabl
        self.timeofeventTabl.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.ResizeToContents)   
        self.timeofeventTabl.verticalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.timeofeventTabl.resizeColumnsToContents()

        #self.timeofeventTabl.setSizeAdjustPolicy(QAbstractScrollArea.sizeAdjustPolicy)
        self.timeofeventTabl.resizeColumnsToContents()
        self.timeofeventTabl.verticalHeader().setVisible(False)
        self.lenT=len(s)
        #self.timeofeventText.setText('\n'.join([str(f'\t{i.split()[0]}\t{i.split()[1]}\t{i.split()[2]}') for i in s]))
        ...
        for i in s:
            ...
            self.ss.append(float(i.split()[5]))
    ...
    def drow(self):
        ...
        print(self.ss)
        cmap = cm.get_cmap()
        norm = Normalize(vmin=min(self.ss), vmax=max(self.ss))
        rgba_values = cmap(norm(self.ss))

        for i in self.buttons:
            if str(i.ch) in self.list_of_ustanovki:
                d = rgba_values[self.list_of_ustanovki.index(str(i.ch))]
#                j=i.ch
                j=self.list_of_ustanovki.index(str(i.ch))
                print(d)
                print((sum(d) - 1) / 3)
                #i.col = 
                #i.col = '#2F4F4F'
                ...
                i.col = matplotlib.colors.rgb2hex(d)
                print(i.col)
#                print(self.ss[j])
#                print(i.ch,j,self.list_of_ustanovki.index(str(i.ch)))
#                print(self.ss[self.list_of_ustanovki.index(str(i.ch))] )
                print(self.ss[j] )
###############
                i.textcol = matplotlib.colors.rgb2hex((0, 0, 0))
                if self.ss[j] < 10:
...
#                    i.col ='#00FF7F'
#                    i.col ='#00FF9A'
                if 100 <= self.ss[j] < 159:
#                    i.col ='#7FFF00'
                    i.col ='#ADFF2D'
                if 159 <= self.ss[j] < 252:
                    i.col ='#FFFF00'
                if 252 <= self.ss[j] < 399:
                    i.col ='#FFD700'
#                    i.col ='#FFA500'
                if 399 <= self.ss[j] < 631:
                    i.col ='#FF8C00'
                if 631 <= self.ss[j] < 1000:
                    i.col ='#FA8072'
                if 1000 <= self.ss[j] < 1585:
                    i.col ='#FF0000'
                    i.textcol = matplotlib.colors.rgb2hex((1, 1, 1))
                if 1585 <= self.ss[j] < 2512:
                    i.col ='#DC143C'
                    i.textcol = matplotlib.colors.rgb2hex((1, 1, 1))
                if 2512 <= self.ss[j] < 3982:
                    i.col ='#8B0000'
                    i.col ='#B22222'
                    i.col ='#C71585'
                    i.textcol = matplotlib.colors.rgb2hex((1, 1, 1))
...
#                i.setText(f'{int(self.ss[self.list_of_ustanovki.index(str(i.ch))])}\n{i.ch}')   
#                i.setText(f'{{i.ch}\n{int(self.ss[j])}}')
#                i.setText(f'{int(self.ss[j])\n{i.ch}')
                i.setText(f'{i.ch}\n{int(self.ss[j])}')   
...
#                i.col = '#ff00ff'
    ...
    def newSize(self):
        ...
        #h = round(H/2)
        for i in self.buttons:
            i.setFixedSize(round(S//1), round(S//1))
...
#                                        "QPushButton:pressed {background-color: #b784a7 ; }")
        ...
        self.qq.setFixedHeight(int(H//1.5))
        self.qqq.setFixedWidth(1)

        self.btn_startL.setFixedWidth(W//11-15)
        self.btn_startR.setFixedWidth(W//11-15)
        self.buttonleft.setFixedWidth(W//11-15)
        self.buttonright.setFixedWidth(W//11-15)
        self.btn_stop.setFixedWidth(W//11-15)
        self.sliderAutoPlayer.setFixedWidth(W//11-15)
        self.timeofeventTabl.setFixedHeight(self.lenT*32)
        self.timeofeventTabl.setFixedWidth(self.timeofeventTabl.horizontalHeader().length()+2)
        #self.timeofeventTabl.setFixedHeight(self.timeofeventTabl.verticalHeader().height() + self.timeofeventTabl.verticalHeader().width())
        print(self.timeofeventTabl.verticalHeader().length())
        print(self.timeofeventTabl.horizontalHeader().length())
    ...
    def openingFile(self, a):
        ...
        try:
            ...
            self.filenameText.setText(f'file: {a}')
        except:
            print('fuck')
...
class Ustanovka(QMainWindow):
    ...
    def __init__(self, b, parent, df, event, fn):
        ...
        self.spinBoxl.setMinimum(self.min)
        self.spinBoxl.setMaximum(self.dlin+self.min)
        self.spinBoxr.setMinimum(self.min)
        self.spinBoxr.setMaximum(self.dlin+self.min)

        self.spinBoxr.setValue(self.parentt.spinBoxR)
        self.spinBoxl.setValue(self.parentt.spinBoxL)
        print(self.parentt.spinBoxR)
        
        print(self.dlin + self.min, '999999')
        self.spinBoxl.valueChanged.connect(self.update_of_spinBoxl)
        self.spinBoxr.valueChanged.connect(self.update_of_spinBoxr)
        self.update_of_spinBoxr()
    ...
    def f1(self, q=0):
        ...
        if self.ba.currentText() == 'Все':
            ...
            self.sc1.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.a1[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='blue', label=f"a1")
            self.sc1.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.a2[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='red', label=f"a2")
            self.sc1.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.a3[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='green', label=f"a3")
            self.sc1.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.a4[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='#FFBF00', label=f"a4")
            if q != 0:
...
#            color = '#00FFFF'
        ...
        self.sc1.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], plot_data[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color=color)
            #self.sc.axes.set_title(f'Event {self.event.currentText()} S{self.b.sipm} C{self.b.ch}')
        self.sc1.axes.grid(linestyle='--', color='pink')
            #self.sc.axes.set_xlim([0, 1023])
        self.sc1.axes.set_xlabel('time')
    ...
    def f2(self, q=0):
        ...
        if self.bd.currentText() == 'Все':
            self.sc2.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.d1[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='blue', label=f"d1")
            self.sc2.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.d2[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='red', label=f"d2")
            self.sc2.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.d3[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='green', label=f"d3")
            self.sc2.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.d4[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='#FFBF00', label=f"d4")
            if q != 0:
...
#            color = '#00FFFF'
        ...
        x = range(0,400)
            #ax.figure(figsize=(7,4))
        self.sc2.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], plot_data[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color=color)
            #self.sc.axes.set_title(f'Event {self.event.currentText()} S{self.b.sipm} C{self.b.ch}')
        self.sc2.axes.grid(linestyle='--', color='pink')
            #self.sc.axes.set_xlim([0, 1023])
        self.sc2.axes.set_xlabel('time')
    ...
    def tr(self):
        ...
        x = range(0,400)
            #ax.figure(figsize=(7,4))
#        self.sc3.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.tr1[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='blue')
        self.sc3.axes.plot(range(qq1, qqq1+qq1)[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], self.tr1[self.spinBoxl.value()-qq1:self.spinBoxr.value()-qq1], color='#8B008B')
            #self.sc.axes.set_title(f'Event {self.event.currentText()} S{self.b.sipm} C{self.b.ch}')
